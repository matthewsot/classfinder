...
/// <reference path="../Scripts/jquery.signalR-1.0.0-rc2.min.js" />
...
class Flauntly {
    ...
    static getUndisplayHeight(item: HTMLElement): number {
    ...
    static linkToggle(item: HTMLElement, forceSelected: bool = false, isLeft: bool = false, forceUnselected: bool = false) {
    ...
    //gets a bool representing whether or not the element is overflowing.
    ...
    public static checkOverflow(el)
    {
...
class topNav {
    ...
    static applyLinks(selected: string[] = new Array(), divID: string = "topNav") {
        $(".link").each(function (): any {
            Flauntly.linkToggle(this, false, false, true);
        });

        $(".rightLink").each(function (): any {
            Flauntly.linkToggle(this, false, false, true);
        });

        $(".leftLink").each(function (): any {
            Flauntly.linkToggle(this, false, false, true);
        });
    ...
    constructor(Layers: string[], RightLayers: number[], LayerIDs: string[] = new Array()) {
        this.contents = new Array();
        for (var layer = 0; layer < Layers.length; layer++) {
            var newHeading: HTMLHeadingElement = <HTMLHeadingElement>document.createElement("h1");
